 ...
 * by whether DATABASE_URL is set — nothing above this file needs to know
 * or care which backend actually answered the call.
 *
 * The Postgres module is only ever imported when DATABASE_URL is present,
 * and vice versa for the SQLite module — dynamic import() means the one
 * not in use is never evaluated, so better-sqlite3's native binary is
 * never touched in production and pg never opens a connection locally.
 */
import type { CreatePostInput, DbBackend } from "./types";

function getBackend(): Promise<DbBackend> {
  return process.env.DATABASE_URL ? import("./postgres") : import("./sqlite");
}
